...
For the surrounding source/configuration/checkpoint packaging workflow, see [Experiment artifacts](../simulation/docs/artifact-lineage.md). The archived Pi4 measurement profile is separate from the Pi Zero2W host named in later commissioning records; the latter is not a deployment result for this standalone package.
...
This public explanation is grounded in the dated project records identified in the [source records for this chapter](../../project/evidence/source-records.md#chapter-software-engineering-md). The catalogue distinguishes complete notices from adapted explanations and preserves separate document versions.
